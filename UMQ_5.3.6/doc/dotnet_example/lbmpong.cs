...
namespace LBMApplication
{
	class lbmpong
	{
		...
		public static void  format(byte[] buf, int offset, long v)
		{
			...
			buf[offset] = (byte) ((((ulong)v) & 0xff00000000000000L) >> 56);
			buf[offset + 1] = (byte) ((v & 0x00ff000000000000L) >> 48);
			buf[offset + 2] = (byte) ((v & 0x0000ff0000000000L) >> 40);
			buf[offset + 3] = (byte) ((v & 0x000000ff00000000L) >> 32);
			buf[offset + 4] = (byte) ((v & 0x00000000ff000000L) >> 24);
			buf[offset + 5] = (byte) ((v & 0x0000000000ff0000L) >> 16);
			buf[offset + 6] = (byte) ((v & 0x000000000000ff00L) >> 8);
			buf[offset + 7] = (byte) (v & 0x00000000000000ffL);
		}
		...
		public static long parse_s(byte[] buf, int offset)
		{
			...
			long v;
			v = (long) convert(buf[offset]) << 56;
			v |= (long) convert(buf[offset + 1]) << 48;
			v |= (long) convert(buf[offset + 2]) << 40;
			v |= (long) convert(buf[offset + 3]) << 32;
			v |= (long) convert(buf[offset + 4]) << 24;
			v |= (long) convert(buf[offset + 5]) << 16;
			v |= (long) convert(buf[offset + 6]) << 8;
			v |= (long) convert(buf[offset + 7]);
	...
	class PongLBMReceiver
	{
		...
		internal int rtt_ignore = 0;
		internal bool use_mim = false;
		...
		protected int onReceive(object cbArg, LBMMessage msg)
		{
			...
			case LBM.MSG_DATA:
				...
				if (_ping)
				{
					...
					{
						rtt_avg_usec = (double) total_usec / (double) _msg_count;
					...
					lbmpong.format(message, 0, System.Diagnostics.Stopwatch.GetTimestamp() * lbmpong.pspertick / 1000);
				}
				if(use_mim)
		...
		internal long total_usec;
		internal long min_nsec, max_nsec = 0;
		internal long min_nsec_idx, max_nsec_idx;
		internal int datanum = 0;
		internal double[] rtt_data;
		internal double rtt_median_msec, rtt_avg_usec, rtt_stddev_msec;
		...
		public virtual void  print_rtt_data()
		{
			if (rtt_data != null)
			{
				int r;
		...
		public void print_latency(System.IO.TextWriter fstr)
		{
			...
			fstr.WriteLine("Elapsed time " + total_usec + " usecs "
