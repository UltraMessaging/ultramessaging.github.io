...
static const char *rcsid_example_umercv = "$Id: //UMprod/REL_5_3_6/29West/lbm/src/example/umercv.c#2 $";
#if defined(_WIN32)
...
const char * OptionString = "1Ac:D:Ee:hi:N:r:s:SU:u:vVx";
...
/* callback for setting the RegID based on src and src RegID (UME 1.2 and earlier) */
lbm_uint_t ume_rcv_regid_compat10(const char *src_str, lbm_uint_t src_regid, void *clientd)
{
	struct Options *opts = &options;

	/* regid should be per src_regid, but we just make this simple for testing */
	lbm_uint_t regid = src_regid + opts->regid_offset;

	if (opts->verbose)
		printf("[%s][%u] Requesting regid: %u\n", src_str, src_regid, regid);
	return regid;
}
...
int main(int argc, char **argv)
{
	...
	if (opts->regid_offset != 0 && opts->compat10) {
		lbm_ume_rcv_regid_func_t id;

		id.func = ume_rcv_regid_compat10;
		id.clientd = NULL;

		if (lbm_rcv_topic_attr_setopt(rcv_attr, "ume_registration_function", &id, sizeof(id)) == LBM_FAILURE) {
			fprintf(stderr, "lbm_rcv_topic_attr_setopt:ume_registration_function: %s\n", lbm_errmsg());
			exit(1);
		}
