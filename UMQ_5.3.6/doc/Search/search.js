...
var searchOutputFrame = window.document;
/*
 *	When Google gets around to fixing the Chrome bug mentioned in doc/Search/util.js
 *	when can re-direct the search links to the content frame.  Until then, in the name
 *	of keeping the experience uniform we're just going to output search links in the 
 *	navigation frame.
 *
 */
/*
if (window.top.frames.length > 0) {
	searchOutputFrame = (isChrome) ? parent.docnavigator.document : parent.doccontent.document;
} else {
	searchOutputFrame = window.document;
}
*/
// ----------------------------------------------------------------------------
...
function PrintHighlightDescription(line)
{
	if (Highlighting == 0)
	{
		searchOutputFrame.writeln(line);
		return;
	}
	...
	searchOutputFrame.writeln(res);
}

function PrintNumResults(num)
{
	var numResultsString = "";

	if (num == 0) {
		numResultsString = STR_NO_RESULTS;
	} else if (num == 1) {
		numResultsString = num + " " + STR_RESULT;
	} else {
		numResultsString = num + " " + STR_RESULTS;
	}
...
{
	document.writeln("<form method=\"get\" action=\"search.html\" class=\"zoom_searchform\">");
}
...
function ZoomShowResultsPerPage()
{
	...
	document.writeln("</select></span><br />");
	document.writeln("</p>");
}
...
{
	document.writeln("<p class=\"zoom_wildcard\">Note that '*' and '?' wildcards are supported.</p>");
...
function ZoomShowHeading()
{
	...
	searchOutputFrame.write("<div class=\"section_title\">" + 
							STR_RESULTS_FOR + " " + queryForHTML);
	if (UseCats) {
		if (cat[0] == -1)
		{
			searchOutputFrame.writeln(" " + STR_RESULTS_IN_ALL_CATEGORIES);
			query_zoom_cats = "&amp;zoom_cat%5B%5D=-1";
		}
		else
		{
			searchOutputFrame.writeln(" " + STR_RESULTS_IN_CATEGORY + " ");
			for (catit = 0; catit < num_zoom_cats; catit++)
			{
				if (catit > 0)
					document.write(", ");
				searchOutputFrame.write("\"" + catnames[cat[catit]] + "\"");
				query_zoom_cats += "&amp;zoom_cat%5B%5D="+cat[catit];
			}
		}
	}
	searchOutputFrame.writeln("</div>");
}
...
function ZoomShowResults()
{
	...
	{
		if (IsWarningGiven == false) {
			document.writeln("<div class=\"results\">This is an advanced template option. You must call " + 
							 "ZoomInitSearch() before this. Please check documentation for more help.</div>");
		}
		IsWarningGiven = true;
		return;
	}
	...
	searchOutputFrame.writeln("<div class=\"results\">");
	...
	while (arrayline < matches && arrayline < result_limit) {
		...
		if (arrayline % 2 == 0)
			searchOutputFrame.writeln("<div class=\"result_block\">");
		else
			searchOutputFrame.writeln("<div class=\"result_altblock\">");
		...
		if (UseZoomImage == 1)
		{
			if (pgimage.length > 1)
			{
				searchOutputFrame.writeln("<div class=\"result_image\">");
				searchOutputFrame.writeln("<a href=\"" + urlLink + "\"" + target + "><img src=\"" + pgimage + "\" alt=\"\" class=\"result_image\" /></a>");
				searchOutputFrame.writeln("</div>");
			}
		}

		searchOutputFrame.writeln("<div class=\"result_title\">");
		if (DisplayNumber == 1)
			searchOutputFrame.writeln("<b>" + (arrayline+1) + ".</b>&nbsp;");

		if (DisplayTitle == 1)
		{
			searchOutputFrame.writeln("<a href=\"" + urlLink + "\"" + target + ">");
			PrintHighlightDescription(pgtitle);
			searchOutputFrame.writeln("</a>");
		}
		else
			searchOutputFrame.writeln("<a href=\"" + urlLink + "\"" + target + ">" + pgurl + "</a>");

		if (UseCats)
		{
			catpage = pageinfo[ipage][PAGEINFO_CAT];
			searchOutputFrame.write("<span class=\"category\">");
			for (cati = 0; cati < NumCats; cati++)
			{
				if (catpage.charAt(cati) == "1")
					searchOutputFrame.write(" ["+catnames[cati]+"]");
			}
			searchOutputFrame.writeln("</span>");
		}
		searchOutputFrame.writeln("</div>");
		...
		if (UseMetaFields == 1 && DisplayMetaFields == 1)
		{
			...
			for (fieldnum = 0; fieldnum < NumMetaFields; fieldnum++)
			{
				...
				if (GetMetaValues(ipage, fieldnum) != null)
				{
					if (metafields[fieldnum][METAFIELD_TYPE] == METAFIELD_TYPE_DROPDOWN)
					{
						searchOutputFrame.writeln("<div class =\"result_custommeta\">");
						searchOutputFrame.writeln("<span class=\""+cssFieldName+"\">"+metafields[fieldnum][METAFIELD_SHOW]+": </span>");
						searchOutputFrame.write("<span class=\""+cssValueName+"\">");
						var ddi = GetMetaValues(ipage, fieldnum);
						searchOutputFrame.writeln(metafields[fieldnum][METAFIELD_DROPDOWN][ddi]+"</span></div>");
					}
					else if (metafields[fieldnum][METAFIELD_TYPE] == METAFIELD_TYPE_MULTI)
					{
						...
						if (metaVal.length > 0)
						{
							...
							{
								if (mvi > 0)
									searchOutputFrame.write(", ");
								searchOutputFrame.write(ddarray[metaVal[mvi]]);
							}
							searchOutputFrame("</span></div>");
						}
					}
					else if (metafields[fieldnum][METAFIELD_TYPE] == METAFIELD_TYPE_MONEY)
					{
						...
						searchOutputFrame.writeln("<span class=\""+cssValueName+"\">"+MetaMoneyCurrency+tmpMoneyStr+"</span></div>");
					}
					else
					{
						searchOutputFrame.writeln("<div class =\"result_custommeta\">");
						searchOutputFrame.writeln("<span class=\""+cssFieldName+"\">"+metafields[fieldnum][METAFIELD_SHOW]+": </span>");
						searchOutputFrame.writeln("<span class=\""+cssValueName+"\">"+GetMetaValues(ipage, fieldnum)+"</span></div>");
					}
				}
			}
		}
		...
		if (DisplayMetaDesc == 1)
		{
			...
			searchOutputFrame.writeln("<div class=\"description\">");
		...
		searchOutputFrame.writeln("<div class=\"infoline\">");
		searchOutputFrame.writeln(info_str);
		searchOutputFrame.writeln("</div></div>\n");
		arrayline++;
	}
	searchOutputFrame.writeln("</div>"); // end results style tag
}
...
function ZoomShowSummary()
{
	...
	{
		if (IsWarningGiven == false)
			searchOutputFrame.writeln("<div class=\"results\">This is an advanced template option. You must call ZoomInitSearch() before this. Please check documentation for more help.</div>");
		IsWarningGiven = true;
		return;
	}
	if (IsNoSearch)
	...
	if (SkippedWords > 0) {
		searchOutputFrame.writeln("<div class=\"summary\">" + STR_SKIPPED_FOLLOWING_WORDS + " " +
									SkippedOutputStr + ".<br /><br /></div>");
	}
	//Display search result information
	searchOutputFrame.writeln("<div class=\"summary\">");
	if (matches == 0) {
		searchOutputFrame.writeln("<br />" + STR_SUMMARY_NO_RESULTS_FOUND + "<br /><br />");	
		searchOutputFrame.writeln("Due to a limitation in our search indexing, we can only");
		searchOutputFrame.writeln("index the first 35 characters of a word.  If you are");
		searchOutputFrame.writeln("searching for an API that is larger than 35 characters, ");
		searchOutputFrame.writeln("please try refining your search.");
	}
	else if (NumSearchWords > 1 && andq == 0) { //OR
		SomeTermMatches = matches - fullmatches;
		searchOutputFrame.writeln(PrintNumResults(fullmatches) + " " + 
									STR_SUMMARY_FOUND_CONTAINING_ALL_TERMS + " ");
		if (SomeTermMatches > 0) {
			searchOutputFrame.writeln("<br />" + PrintNumResults(SomeTermMatches) + " " + 
										STR_SUMMARY_FOUND_CONTAINING_SOME_TERMS);
		}
		searchOutputFrame.writeln("<br />");
	}
	else if (NumSearchWords > 1 && andq == 1) {//AND
		searchOutputFrame.writeln(PrintNumResults(fullmatches) + " " + 
									STR_SUMMARY_FOUND_CONTAINING_ALL_TERMS + "<br />");
	} else {
		searchOutputFrame.writeln(PrintNumResults(matches) + " " + STR_SUMMARY_FOUND + "<br />");
	}
	searchOutputFrame.writeln("</div>\n");
}
	...
	for (catit = 0; catit < NumCats; catit++)
	{
		if (CatCounter[catit] > 0)
		{
			...
			if (CatCounter[catit] != matches)
			{
				...
				{
					searchOutputFrame.writeln("<div class=\"cat_summary\"><br />" + STR_CAT_SUMMARY + "<ul>");				
					ShowCatSummaryHeading = false;
					CatSummaryShown = true;
				}
				searchOutputFrame.writeln("<li><a href=\"" + SelfURL + LinkBackJoinChar + "zoom_query=" + queryForURL + metaParams + "&amp;zoom_cat%5B%5D=" + catit + "&amp;zoom_per_page=" + per_page + "&amp;zoom_and=" + andq + "&amp;zoom_sort=" + sort + "\">" + catnames[catit]);
				searchOutputFrame.writeln("</a> (" + CatCounter[catit] + ")</li>");
			}
		}
	}
	if (DisplayCatSummary == 1 && CatSummaryShown == true)
		searchOutputFrame.writeln("</ul></div>");
}
...
function ZoomShowPagesCount()
{
	...
	/*if (num_pages > 1)
		searchOutputFrame.writeln("<div class=\"result_pagescount\"><br />" + num_pages + " " + STR_PAGES_OF_RESULTS + "</div>\n");
	*/
}
...
function ZoomShowPageNumbers()
{
	...
	{
		if (IsWarningGiven == false) {
			document.writeln("<div class=\"results\">This is an advanced template option. " + 
							 "You must call ZoomInitSearch() before this. Please check documentation" + 
							 " for more help.</div>");
		}
		IsWarningGiven = true;
		return;
	...
	if (num_pages > 1) {
		// 4 results to the left of the current page
		start_range = page - MAX_RANGE;
		if (start_range < 1) {
			start_range = 1;
		}
		
		// 4 to the right
		end_range = page + MAX_RANGE;
		if (end_range > num_pages) {
			end_range = num_pages;
		}
			
		searchOutputFrame.writeln("<div class=\"result_pages\"><div class=\"result_location\">Page " + page + 
								  " of " + num_pages + "</div>");

		for (i = start_range; i <= end_range; i++) {
			if (i == page) {
				searchOutputFrame.writeln("<div class=\"current\">" + page + "</div>");
			}
			else {
				searchOutputFrame.writeln("<a href=\"" + SelfURL + LinkBackJoinChar + 
											 "zoom_query=" + queryForURL + metaParams + "&amp;" + 
											 "zoom_page=" + i + "&amp;" + 
											 "zoom_per_page=" + per_page + query_zoom_cats + "&amp;" + 
											 "zoom_and=" + andq + "&amp;" + 
											 "zoom_sort=" + sort + "\">" + 
											 i + 
											 "</a> ");
		...
		// Show a drop down menu of all page numbers to select from
		if (showDrowDown) {
			var linkValue;
			if (num_pages > (MAX_RANGE * 2)) {
				
				document.writeln("<form id =\"page_select\" name=\"page_select\" " + 
								  "action=\"search.html\" " + 
								  "method=\"GET\" " + 
								  "onclick=\"return dropdown(page_select.value)\">");
				document.writeln("<select name=\"page_select\">");
		
				for (i = 1 ; i <= num_pages - 1 ; i++) {
	 				linkValue = SelfURL + LinkBackJoinChar + 
							 	"zoom_query=" + queryForURL + metaParams + "&amp;" + 
							 	"zoom_page=" + i + "&amp;" + 
							 	"zoom_per_page=" + per_page + query_zoom_cats + "&amp;" + 
							 	"zoom_and=" + andq + "&amp;" + 
							 	"zoom_sort=" + sort;
					document.writeln("<option value=\"" + linkValue + "\">" + i + "</option>");
				}
				// Last page is the default selected value
				linkValue = SelfURL + LinkBackJoinChar + 
							 	"zoom_query=" + queryForURL + metaParams + "&amp;" + 
							 	"zoom_page=" + i + "&amp;" + 
							 	"zoom_per_page=" + per_page + query_zoom_cats + "&amp;" + 
							 	"zoom_and=" + andq + "&amp;" + 
							 	"zoom_sort=" + sort;
				document.writeln("<option value=\"" + linkValue + "\" selected>" + i + "</option>");
...
function ZoomShowSearchTime()
{
	...
	if (Timing == 1)
		document.writeln("<div class=\"searchtime\"><br /><br />" + 
						 STR_SEARCH_TOOK + " " + (timeDifference/1000) + 
						 " " + STR_SECONDS + ".</div>\n");
}

function dropdown(newURL) {
...
function ZoomSearch()
{
	ZoomInitSearch();
	ZoomShowSearchForm();
	searchOutputFrame.writeln("<div id=\"results_body\">");
	ZoomShowHeading();
	ZoomShowSummary();
	ZoomShowCatSummary();
	ZoomShowPagesCount();
