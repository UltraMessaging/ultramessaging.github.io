/*file: q-example-src.c - UME Queue example source program.
 *
 ...
 * according to the terms laid out in the Software License Agreement.
 ...
 -           -L$HOME/lbm/LBM_<VERS>/<PLATFORM>/lib -llbm -lm -o q-example-src q-example-src.c
...
main()
{
    ...
    int message_num = 1;        /* counter used for sending messages */
    char message[64];           /* buffer to hold message that will be sent */
    char queue_name[64] =       /* name of the queue to submit messages to */
        "Queue Training";
     ...
     * Set the umq_queue_name attribute to use the Queue name desired
     */
     ...
     */
    err = lbm_src_topic_alloc(&topic, ctx, "UME Queue Example", attr);
    if (err)
    {
        printf("line %d: %s\n", __LINE__, lbm_errmsg());
        exit(1);
     ...
     * for details. In addition, for Queue sources, we have to give a source
     * time to register with Queues. We could check source events if we
     * desired to know when we could send or not. But here, we will just pause
     */
    SLEEP(3);

    /* Send 20 messages with a 1 second pause between them */
    for (; message_num <= 20; message_num++)
    {
        /* Create a unique message for this iteration */
        sprintf(message, "UME Queue Message %02d", message_num);

        /*callout: send
         * Send a message to the "UME Queue Example" topic.  The flags make sure the
         * call to lbm_src_send doesn't return until the message is sent.
         */
        err = lbm_src_send(src, message, 20, LBM_MSG_FLUSH | LBM_SRC_BLOCK);
        if (err)
        {
            printf("line %d: %s\n", __LINE__, lbm_errmsg());
            exit(1);
        ...
        /* Print that we sent a message to provide some feedback */
        printf("Sent Message %02d\n", message_num);
