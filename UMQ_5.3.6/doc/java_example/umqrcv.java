...
class umqrcv {
	...
	private static String usage = "Usage: umqrcv [options] topic\n"
			+ "Available options:\n"
	...
	public static void main(String[] args) {
		...
		Getopt gopt = new Getopt("umqrcv", args, "+c:DeEi:I:r:R:s:Shqv1X:N:", longopts);
		int c = -1;
		boolean error = false;
...
class UMQRcvCtxCB implements LBMContextEventCallback
{
	...
	public UMQRcvCtxCB(boolean verbose)
	{
	      _verbose = verbose;
	}
...
class UMQRcvReceiver implements LBMReceiverCallback,
	...
	public UMQRcvReceiver(boolean verbose, boolean end_on_eos, boolean summary) {
		_verbose = verbose;
		...
		case LBM.MSG_DATA:
			...
			if (_verbose) {
				...
				if (msg.queueMessageId() != null) {
					System.err.print("[" 
							+ msg.queueMessageId().registrationId().toString() 
							+ ":" 
							+ msg.queueMessageId().msgStamp().toString()
							+ "]");
					try {
						if (msg.queueIndexInfo() != null)
						{
							System.err.print("[" + (((msg.queueIndexInfo().flags() & LBM.UMQ_INDEX_FLAG_NUMERIC) != 0) ? msg.queueIndexInfo().numericIndex() : "\"" + new String(msg.queueIndexInfo().index()) + "\"") + "]");
						}
					} catch (LBMException e) {
						e.printStackTrace();
					}
		...
		case LBM.MSG_UMQ_INDEX_ASSIGNED_EX:
			...
			try {
				System.out.println("["
						...
						+ "] assigned index "
						+ (((ia.indexInfo().flags() & LBM.UMQ_INDEX_FLAG_NUMERIC) != 0) ? ia.indexInfo().numericIndex() : "\"" + new String(ia.indexInfo().index()) + "\"")
						+ ". Flags "
